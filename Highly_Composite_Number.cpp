...
ll n, res_num, res_div;

void call(int pos, int lmt, ll cur_num, ll cur_div) {
	if(cur_div > res_div) {
		res_num = cur_num;
		res_div = cur_div;
	}
	else if(cur_div == res_div && cur_num < res_num) {
		res_num = cur_num;
	}
	if(pos == 9) return;
	int p = prime[pos];
	for(int i = 1; i <= lmt; i++) {
		if(cur_num * p > n) break;
		call(pos + 1, i, cur_num * p, cur_div * (i + 1));
		p *= prime[pos];
	}
}
